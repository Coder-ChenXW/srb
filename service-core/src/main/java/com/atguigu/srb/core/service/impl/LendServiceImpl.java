...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    public void makeLoan(Long id) {
        ...
        lend.setStatus(LendStatusEnum.PAY_RUN.getStatus()); // 放款中
...
//                    result.getString("agentBillNo"),
    ...
    private void repaymentPlan(Lend lend) {
        ...
        for (int i = 1; i <= len; i++) {
            ...
            lendReturn.setReturnDate(lend.getLendStartDate().plusMonths(i)); // 第一期还款时间 = 放款时间 + 1个月
    ...
    public List<LendItemReturn> returnInvest(Long lendItemId, Map<Integer, Long> lendReturnMap, Lend lend) {
        ...
        for (Map.Entry<Integer, BigDecimal> entry : mapInterest.entrySet()) {
            ...
            if (currentPeriod.intValue() == lend.getPeriod().intValue()) {
                ...
                BigDecimal lastPrincipal = lendItem.getExpectAmount().subtract(sumPrincipal);
                lendItemReturn.setInterest(lastPrincipal);
